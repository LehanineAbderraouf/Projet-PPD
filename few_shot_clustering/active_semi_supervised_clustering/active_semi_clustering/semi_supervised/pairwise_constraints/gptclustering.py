...
Keyphrases:"""
    ...
    def fit(self, X):
        ...
        for doc in self.documents:
            ...
            #all_expansions.append(doc_expansions[0])
        ...
        if self.algorithm == "KMeans":
            ...
            #clusterer = KMeans(self.n_clusters, max_iter=100, init="k-means++", normalize_vectors=True, split_normalization=True, split_point=np.shape(b_vectors)[1])
            #clusterer = KMedoids(n_clusters=self.n_clusters, random_state=0).fit(b_vectors)
        ...
        # centroids_indexes = [int(index) for index in clusterer.medoid_indices_]
        # self.centroids = [all_expansions[i] for i in  centroids_indexes ]
        return self
